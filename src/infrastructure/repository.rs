...
mod tests {
    ...
    fn ten_thousand_tasks_can_be_saved_and_loaded() {
        ...
        assert!(
            ...
            "10,000 task round trip took {:?}",
            started.elapsed()
        );
        assert!(
            load_elapsed < Duration::from_secs(1),
            "10,000 task load took {load_elapsed:?}"
        );
        ...
        assert!(
            search_started.elapsed() < Duration::from_millis(100),
            "10,000 task search took {:?}",
