...
- 上部の「新規タスク」欄へタスク名を入力し、「タスクを保存」またはEnterで作成します。
- 左側でスマートビュー、プロジェクト、タグ、保存済みビューを選びます。
- サイドバーの右端を左右へドラッグして幅を変更できます。完了、ゴミ箱などは「その他のビュー」から展開します。
- 中央でリスト／ボード／カレンダーを切り替えます。
- タスクを選ぶと、中央上部でタイトル、納期、メモを編集できます。「状態・優先度など」を開くと進捗、プロジェクト、タグなども変更できます。
- タイトルとメモは自動保存され、「変更を保存」で明示的に保存することもできます。保存に成功すると編集カードは閉じ、入力エラー時は開いたままになります。
...
- 納期は空欄、`YYYY-MM-DD`、`YYYY-MM-DD HH:MM`のいずれかを入力し、Enterまたは「変更を保存」で確定します。
