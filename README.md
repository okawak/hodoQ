...
- PRを閉じたときとCIの終了後に、閉じたPRの`refs/pull/<番号>/merge`だけを対象にキャッシュを削除します。PRを閉じた後でCIが保存したキャッシュも回収し、`main`や開いているPRのキャッシュを残します。この処理は`pull_request_target`と`workflow_run`で動きますが、PRのコードや成果物を取得・実行しません。権限はキャッシュ削除用の`actions: write`とPR状態確認用の`pull-requests: read`だけを付与し、複数の削除処理は直列化します。
