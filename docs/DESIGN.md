...
| ビュー切り替え・保存ビュー・フィルター入力・表示タスクの抽出 | `workspace/views.rs` |
...
責務分割ではDBスキーマとJSON/CSVの形式を維持する。
復元では読み取り専用で開いた元DBをメモリへコピーし、バージョン・整合性・マイグレーション・全エンティティの読み取り・外部キーを検証する。さらに正規のマイグレーションで作った別のメモリDBへ全テーブルの値を挿入し、CHECK・UNIQUE・外部キー制約を検証する。復元元のDDL・トリガーは引き継がない。検証後に現在のDBを退避し、SQLite Backup APIで正規スキーマのDBを適用する。検証失敗時は現在のDBと復元元を変更しない。

編集途中のタイトル・メモは、保存に成功するまでUIに保持する。無効なタイトルも未保存状態として記録し、古いデバウンス処理を無効にする。タスク・ビューの切り替え、複製、編集中のタスクをゴミ箱へ移す操作、フォームを閉じる操作は保留中の保存結果を確認し、失敗時は編集対象と入力を維持する。新規フォームを開いている間の複製は、作成途中の入力を保持するため編集対象を切り替えない。
