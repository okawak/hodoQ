...
- `infrastructure/repository.rs`: 接続の初期化と共通の入口。タスクSQL、分類・保存ビュー、バックアップ・出力、行変換は責務ごとの子モジュールに分ける。
...
エラーは`Result`で伝播する。SQL・I/O等の詳細エラーはinfrastructureに閉じ、アプリケーション境界で表示可能なエラーへ変換する方針とする。
